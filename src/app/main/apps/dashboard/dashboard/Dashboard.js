	...
	useEffect(() => {
		if (!_.isEmpty(widgets) && !_.isEmpty(main)) {
			...
			if (widgets.Dashboard_Multiline_GoalAndActual_Auto_Panel) {
				...
				users.map(user => {
					if (user.id === UID) {
						policies.map(policy => {
							if (policy.value !== 'Total') {
								// indGoalsAndActual[`Total@realGoal`] +=
								// 	main[production][period][user.id][policy.value]['realGoal'];
								indGoalsAndActual[`Total@Goal`] +=
									main[production][period][user.id][policy.value]['Goals'];
								indGoalsAndActual[`Total@Actual`] +=
									main[production][period][user.id][policy.value]['Policies'];
								// indGoalsAndActual[`${policy.value}@realGoal`] +=
								// 	main[production][period][user.id][policy.value]['realGoal'];
								indGoalsAndActual[`${policy.value}@Goal`] +=
									main[production][period][user.id][policy.value]['Goals'];
					...
					if (user.id === belongTo) {
						policies.map(policy => {
							if (policy.value !== 'Total') {
								// teamGoalsAndActual[`Total@realGoal`] +=
								// 	main[production][period][user.id][policy.value]['realGoal'];
				...
				policies.map(policy => {
					...
					cardData.map(card => {
						...
						// if (
						// 	teamGoalsAndActual[`${policy.value}@real${card.label}`] >
						// 	teamGoalsAndActual[`${policy.value}@${card.label}`]
						// ) {
						// 	color = 'text-red';
						// }
						// if (card.label === 'Goal') {
						// 	tempCard = {
						// 		...tempCard,
						// 		count: indGoalsAndActual[`${policy.value}@real${card.label}`],
						// 		color: color
						// 	};
					...
					widgets = {
						...widgets,
						[`Dashboard_Multiline_GoalAndActual_${policy.value}_Panel`]: {
				...
				policies.map(policy => {
					...
					cardData.map(card => {
						...
						// 	tempCard = {
						// 		...tempCard,
						// 		count: teamGoalsAndActual[`${policy.value}@real${card.label}`],
						// 		color: color
						// 	};
						// } else if (card.label === 'Actual') {
			...
			if (widgets.Dashboard_Personal_GoalVsActual_Chart) {
				...
				widgets = {
					...widgets,
					Dashboard_Personal_GoalVsActual_Chart: {
