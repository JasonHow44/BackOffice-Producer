	...
	function onSave() {
		...
		if (checkValidation()) {
			...
			if (state.datePolicyIsIssued) {
				if (
					state.typeOfProduct === 'Term' ||
					state.typeOfProduct === 'Limited Pay' ||
					state.typeOfProduct === 'Final Expense'
				) {
					form = {
						...form,
						dollarBonus:
					...
					state.typeOfProduct === 'Whole Life' ||
					state.typeOfProduct === 'Universal Life' ||
					state.typeOfProduct === 'UL Bump'
				) {
					form = {
						...form,
						dollarBonus:
			...
			if (state.paymentMode === 'Monthly') {
				if (state.monthPremiumCredit) {
					form = {
						...
						lifePolicyPremiumCredit:
							Math.ceil(
								(parseFloat(state.policyPremium) / 12) * (13 - parseInt(state.monthPremiumCredit)) * 100
							) / 100
					};
				}
			}
